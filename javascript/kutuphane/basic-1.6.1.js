...
// Oluşturulan nesnelerin, ekleneceği kutu.
basic.selectedBox;
...
basic.autoAddObjectIntoBoxController.boxList = [];

// Nesnelerin tarayıcı tarafından hesaplanmış boyutlarını kontrol eder.
basic.autoSizedObjectController = {}
basic.autoSizedObjectController.DELAY_TIME = 5;
basic.autoSizedObjectController.MIN_NUMBER_OF_CHECK = 4;
basic.autoSizedObjectController.timer = 0;
basic.autoSizedObjectController.objectList = [];
basic.autoSizedObjectController.afterSizedFunctionList = [];

// Hazır olan nesnelerin görünmesini sağlar.
basic.createdObjectVisibilityController = {}
basic.createdObjectVisibilityController.DELAY_TIME = 
    basic.autoSizedObjectController.DELAY_TIME;
...
// Rasgele bir sayı üretir. (Tek parametre izin verilmiyor.)
...
class Box extends Basic_UIComponent {
    ...
    set height($value) {
        if ($value != "auto") {
            ...
            basic.autoSizedObjectController.pushIntoList(this, "height");
        }
    }
...
class Button extends Basic_UIComponent {
    ...
    set width($value) {
        if ($value != "auto") {
            ...
            basic.autoSizedObjectController.pushIntoList(this, "width");
        }
    }
...
let afterSized = function($func, $object) {
    ...
    basic.autoSizedObjectController.afterSizedFunctionList.push({
        func: $func,
        obj: $object
    });

    if($object) {
        basic.autoSizedObjectController.pushIntoList($object, "width");
        basic.autoSizedObjectController.pushIntoList($object, "height");
...
// Otomatik boyutlandırılmış nesneyi kontrol edilmek üzere, listeye ekle.
basic.autoSizedObjectController.pushIntoList = function ($object, $sizeType) {

    var isSizeTypeOfObjectPushedBefore = 0;

    for(var i = 0; i < basic.autoSizedObjectController.objectList.length; i++) {
        if (basic.autoSizedObjectController.objectList[i].object == $object &&
            basic.autoSizedObjectController.objectList[i].type == $sizeType) {
    ...
    if (!isSizeTypeOfObjectPushedBefore && 
        ...
        basic.autoSizedObjectController.objectList.push({
            object: $object, 
            type: $sizeType, 
            value: 0, 
            sized: 0
        ...
        basic.autoSizedObjectController.callAfterSizedFunctions();

    }

}

// Otomatik boyutlandırılmış nesnelerin, tanımlanmış fonksiyonlarını çağır. (Hazır olduğunda.)
basic.autoSizedObjectController.callAfterSizedFunctions = function () {

    if (basic.autoSizedObjectController.timer) {
        basic.autoSizedObjectController.timer = 
            clearTimeout(basic.autoSizedObjectController.timer);
    }

    basic.autoSizedObjectController.timer = setTimeout(function () {

        // Hepsi hesaplanmış ise,
        if (basic.autoSizedObjectController.areObjectsReady()) {
            ...
            for (var i = 0; i < basic.autoSizedObjectController.afterSizedFunctionList.length; i++) {
                basic.autoSizedObjectController.afterSizedFunctionList[i]
                    .func(basic.autoSizedObjectController.afterSizedFunctionList[i].obj);
            }
...
// Otomatik boyutlandırılmış nesnelerin, hesaplanması tamamlanmış mı?
basic.autoSizedObjectController.areObjectsReady = function() {
    ...
    for (var i = 0; i < basic.autoSizedObjectController.objectList.length; i++) {
        ...
        var item = basic.autoSizedObjectController.objectList[i];
        ...
        if (item.sized >= basic.autoSizedObjectController.MIN_NUMBER_OF_CHECK) {
            countOfSizedObjects++;
        }
    ...
    // Hepsi hazır ise,
    if (countOfSizedObjects == basic.autoSizedObjectController.objectList.length) {
        return 1;
...
// Denetçi şu anda çalışıyor mu?
basic.autoSizedObjectController.isRunning = function() {

    var isRunning = 0;

    // Eğer, nesnelerin otomatik boyut hesaplaması bitmiş ise,
    if (basic.autoSizedObjectController.objectList.length > 0) {
        isRunning = 1;
    }
...
basic.createdObjectVisibilityController.makeVisibleWhenReady = function ($object) {
    ...
    controller.timer = setTimeout(function() {
        ...
        howManyControllerAreRunning += basic.autoSizedObjectController.isRunning();
