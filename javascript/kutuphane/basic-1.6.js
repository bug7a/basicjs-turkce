...
basic.loopTimer;
// loop fonksiyonunu sürekli çağıran timer.
basic.autoAddObjectIntoBoxTimer;
// Otomatik boyutlandırma için kullanılan timer.
basic.autoSizedObjectsControlTimer;
// Yeni oluşturulan nesneler bir süre görünmezdir.
basic.unvisibleObjectsControlTimer;

// Sayfaya eklenen tüm box nesneleri.
basic.createdBoxList = [];
// Boyutu otomatik belirlenen nesnelerin listesi.
basic.autoSizedObjectControlList = [];
// Otomatik boyutlandırma tamamlandıktan sonra, çalıştırılacak fonksiyonlar.
basic.afterSizedFunctionList = [];
// Yeni oluşturulan ve görünmeyen nesnelerin listesi.
basic.unvisibleObjectControlList = [];

// Ayarlar.
basic.settings = {};
// Nesnelerin, kutuların içine otomatik taşınmasına izin ver.
basic.settings.AUTO_ADD_OBJECTS_INTO_BOXES = 1;

// Oluşturulan nesneler kaç ms sonra otomatik taşınsın.
basic.settings.AUTO_ADD_OBJECT_INTO_BOX_TIME = 1;
// Nesnelerin kontrol döngüleri:
basic.settings.AUTO_SIZED_OBJECTS_CONTROL_TIME = 5;
basic.settings.AUTO_SIZED_OBJECT_MIN_CONTROL_NUMBER = 4; // 12 ok

basic.settings.UNVISIBLE_OBJECTS_CONTROL_TIME = basic.settings.AUTO_SIZED_OBJECTS_CONTROL_TIME;

// Hareketi tanımlama ve başlama süreleri.
basic.settings.SET_MOTION_TIME = 80;
// NOT: Hareket tanımlanan nesneler, istenmediğinde; tanımlama satırlarındaki değişiklikleri harekete çevirmez.
basic.settings.WITH_MOTION_TIME = 100;
// NOT: Hareket edecek nesneler, belli bir süre boyunca, ilk konumlarında bekler. 
// Bu, hareketin başını kaçırdığın hissini yumuşatıyor.
...
class Box extends Basic_UIComponent {
    ...
    constructor($left = 0, $top = 0, $width = basic.BOX_WIDTH, $height = basic.BOX_HEIGHT) {
        ...
        basic.makeVisibleWhenReady_newCreatedObject(this);
    ...
    set height($value) {
        if ($value != "auto") {
            ...
            basic.pushInto_autoSizedObjectControlList(this, "height");
        }
    }
...
class Button extends Basic_UIComponent {
    ...
    set width($value) {
        if ($value != "auto") {
            ...
            basic.pushInto_autoSizedObjectControlList(this, "width");
        }
    }
...
basic.moveToCenter = function ($this, $position) {
    ...
    if (basic.isAllowed_autoAddObjectIntoBox()) {
        basic.autoAddObjectIntoBox($this);
...
let makeBasicObject = function($newObject) {
    ...
    if (basic.isAllowed_autoAddObjectIntoBox() && that) {
        basic.autoAddObjectIntoBox(that);
    ...
    if (basic.isAllowed_autoAddObjectIntoBox()) {
        basic.autoAddObjectIntoBox_afterLinkedWithVariable(that);
...
basic.autoAddObjectIntoBox_afterLinkedWithVariable = function ($object) {

    // Bir nesne create edildikten sonra, bir box nesnesinin içine aktarılıyor. (gecikme)
    // page1.btn1 = createButton() gibi,
    
    var timerName = "autoAddObjectIntoBoxTimer";

    // Yeni nesne eklendiğinde bir önceki otomatik eklenir, zamanlayıcıyı temizle.
    if (basic[timerName]) {
        basic[timerName] = clearTimeout(basic[timerName]);
    }

    // Son eklenen nesne için uygula.
    basic[timerName] = setTimeout(function () {
        basic.autoAddObjectIntoBox($object);
    }, basic.settings.AUTO_ADD_OBJECT_INTO_BOX_TIME);

}

// Nesnenin taşınması gerektiğini tespit eden ve taşıyan fonksiyon.
basic.autoAddObjectIntoBox = function ($object) {
...
// isAllowed_autoAddObjectsIntoBoxes
basic.isAllowed_autoAddObjectIntoBox = function () {
    return basic.settings.AUTO_ADD_OBJECTS_INTO_BOXES;
}

// Nesne boyutu hesaplandıktan sonra, verilen fonksiyonu çağırır
let afterSized = function($func, $object) {

    basic.afterSizedFunctionList.push({
        func: $func,
        obj: $object
    });

    if($object) {
        basic.pushInto_autoSizedObjectControlList($object, "width");
        basic.pushInto_autoSizedObjectControlList($object, "height");
        // NOT: Eğer, bir nesne için, autoSized() fonksiyonu çağırılırsa,
        // nesnenin, yüksekli ve genişliğini tekrar kontrol listesine ekle.
        // Mesela: .text değiştirilmiş ve genişlik tekrar talep ediliyor olabilir.
    }
    // NOT: aftersized() nesne belirtmeden de kullanılabilir:
    // O zaman, boyutu otomatik olarak belirtilen tüm nesneler hazır olduğunda çağırılır.
    // Eğer, otomatik boyutlandırılmış hiç bir nesne yok ise çağırılmaz.

}

basic.pushInto_autoSizedObjectControlList = function ($object, $sizeType) {

    var objectListName = "autoSizedObjectControlList";
    var isSizeTypeOfObjectPushedBefore = 0;

    for(var i = 0; i < basic[objectListName].length; i++) {
        if (basic[objectListName][i].object == $object &&
            basic[objectListName][i].type == $sizeType) {
                isSizeTypeOfObjectPushedBefore = 1;
        }
    }

    var isSizeTypeOfObjectAuto = 0;

    if ($object.contElement.style[$sizeType] == "auto") {
        isSizeTypeOfObjectAuto = 1;
    }

    if (!isSizeTypeOfObjectPushedBefore && 
        isSizeTypeOfObjectAuto) {
        basic[objectListName].push({
            object: $object, 
            type: $sizeType, 
            value: 0, 
            sized: 0
        ...
        basic.call_afterSizedFunctions();

    }

}

// Otomatik boyutlandırılan nesnelerin, boyutları hesaplandı mı? Kontrol et.
// controlAutoSizedObjects_andCallAfterSizedFunctions
// callAfterSizedFunctions
// controlAndCallAfterSizedFunctions
basic.call_afterSizedFunctions = function () {

    var functionListName = "afterSizedFunctionList";
    var timerName = "autoSizedObjectsControlTimer";

    // Zamanlayıcıyı temizle. Sadece 1 zamanlayıcı aktif olabilir.
    if (basic[timerName]) {
        basic[timerName] = clearTimeout(basic[timerName]);
    }
    ...
    basic[timerName] = setTimeout(function () {
        ...
        // Hepsi hesaplanmış ise,
        if (basic.areReady_autoSizedObjects()) {

            print("basic.js: auto calculated time: " + (Date.now() - basic.startTime) + "ms");

            // Tüm nesneler boyutlandırıldı.
            // afterSized() ile gönderilen fonksiyonlarını çağır.
            for (var i = 0; i < basic[functionListName].length; i++) {
                basic[functionListName][i].func(basic[functionListName][i].obj);
            }

            // Görev tamamalandı, listeleri temizle.
            basic.autoSizedObjectControlList = [];
            basic[functionListName] = [];

        } else {
            // Otomatik boyutlar hazır değil, tekrar kontrol edilecek:
            basic.call_afterSizedFunctions();
        }

    }, basic.settings.AUTO_SIZED_OBJECTS_CONTROL_TIME);

}

basic.areReady_autoSizedObjects = function() {

    var objectListName = "autoSizedObjectControlList";
    var countOfSizedObjects = 0;

    // Kontrol et.
    for (var i = 0; i < basic[objectListName].length; i++) {

        var item = basic[objectListName][i];

        if (item.object[item.type] == item.value) {
            item.sized += 1;
        } else {
            item.value = item.object[item.type];
            item.sized = 0;
        }

        // Kontrolden geçmiş ise,
        if (item.sized >= basic.settings.AUTO_SIZED_OBJECT_MIN_CONTROL_NUMBER) {
            countOfSizedObjects++;
        }
    ...
    // Hepsi hazır ise,
    if (countOfSizedObjects == basic[objectListName].length) {
        return 1;
...
// Nesneler hazır olduğunda, hepsini göster.
// pushInto_unvisibleObjectControlList_andControl_andVisibleAllWhenReady
// pushInto_unvisibleObjectControlList_andVisibleAllWhenReady
// showAllObjectsWhenReady
// makeVisibleObjectWhenReady
// pushInto_unvisibleObjectControlList_andMakeVisibleWhenReady
// makeVisibleWhenReady
// makeVisibleFirstTimeWhenReady_newCreatedObject
basic.makeVisibleWhenReady_newCreatedObject = function ($object) {

    var objectListName = "unvisibleObjectControlList";
    var timerName = "unvisibleObjectsControlTimer";

    if ($object) {
        basic[objectListName].push($object);
    }
    ...
    basic[timerName] = setTimeout(function() {
        ...
        // Eğer, nesnelerin otomatik boyut hesaplaması bitmiş ise,
        if (basic.autoSizedObjectControlList.length == 0) {
        ...
        if (areReadyToMakeVisible) {
            ...
            for ( var i = 0; i < basic[objectListName].length; i++) {
                ...
                if (basic[objectListName][i]._visible == 0) {
                    basic[objectListName][i].visible = 0;
                }
                // NOT: Nesne, başlangıçta display:none yapılırsa, "auto" hesaplamalar da yapılamaz.
                // Bu sebeple, nesneler hazır olana kadar; kullanıcının display:none yapma isteğini geciktirir.
                // object.visible = 0 kodu, display özelliğini none yapar.

                basic[objectListName][i].contElement.style.visibility = "visible";

            }
            
            basic[objectListName] = [];
            print("basic.js: show time: " + (Date.now() - basic.startTime) + "ms");

        } else {

            // Hazır değil, tekrar kontrol et:
            basic.makeVisibleWhenReady_newCreatedObject();

        }

    }, basic.settings.UNVISIBLE_OBJECTS_CONTROL_TIME);
